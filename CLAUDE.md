...
│   ├── sitemap.ts          # Sitemap dynamique (auto-inclut blogPosts + wizardVariants)
│   ├── robots.ts           # robots.txt
│   ├── blog/
│   │   ├── page.tsx        # Liste des articles (34 articles, groupés par catégorie)
...
├── data/
│   ├── blogPosts.ts        # Métadonnées des 34 articles (slug, title, category, relatedSlugs)
...
- Crédit (5) — construire crédit, sans historique, après faillite
- Situation Spéciale (4) — réfugié, travailleur autonome, conjoint
- Processus (3) — préapprobation, documents, étapes
- Financement (5) — mise de fonds, RAP/CELIAPP, transfert fonds étranger
- Achat (4) — duplex/triplex, frais cachés, assurance habitation
- Taux (1) — fixe vs variable
- Gestion (1) — renouvellement
- Légal (2) — interdiction achat non-canadien, courtier vs banque

### Sitemap
Dynamique (`src/app/sitemap.ts`), auto-inclut tous les `blogPosts` et `wizardVariants`. Pas besoin de mise à jour manuelle quand on ajoute des articles.
...
- Liens internes dans articles : composant `<InternalLink href="..." anchor="..." />`
